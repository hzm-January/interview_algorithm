...
注：持有股票状态`dp[i][1]=max(dp[i-1][1], 0-prices[i])`，题目要求只能买卖一次，所以首次买入股票时手头金额一定是初始值0，没有递推关系。\
注：如果该题定义三个状态，则持有股票状态`dp[i][2]=max(dp[i-1][2], 0-prices[i])`可以写为`dp[i][2]=max(dp[i-1][2], dp[i][0]-prices[i])`。
前者只是将状态1省略了。
> 状态1：未进行过任何操作；\
> 状态2：第一次持有；\
> 状态3：第一次不持有；\
...
> 状态1：未进行过任何操作；\
> 状态2：只进行过一次买操作；\
> 状态3：进行了一次买操作和一次卖操作，即完成了一笔交易；\
> 状态4：在完成了一笔交易的前提下，进行了第二次买操作；

#### leetcode 0309 买卖归家的最佳时机含冷冻期\
注：本题dp数组，包含4个状态（其中`状态1：未进行过任何操作`状态可以省略）\
注：`leetcode 0122 买卖股票的最佳时机2`中`不持有状态`在该题中需要拆分为`已卖出不持有`与`今天卖出`两个状态，以处理`冷冻期`状态
> 状态1：未进行过任何操作；\
> 状态2：持有；\
> 状态3：不持有；\
> 状态4：卖出；\
